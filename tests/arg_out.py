...
fn test() {
	...
	let (c, b) = my_test::out_values_function_call(2, 3);
	assert_eq!(*c, 16);
	assert_eq!(*b, 42);

	let (r, c, b) = my_test::out_values_function_call_rval(2);
	assert_eq!(r, 2);
	assert_eq!(*c, 16);
	assert_eq!(*b, 28);

	let (r, c, b) = my_test::out_values_function_call_rval_with_k(2, 2);
	assert_eq!(r, 4);
	assert_eq!(*c, 16);
	assert_eq!(*b, 28);
